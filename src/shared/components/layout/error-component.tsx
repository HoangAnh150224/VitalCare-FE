    ...
    if (resource && action) {
      setErrorMessage(
        translate(
          ...
          `Có thể bạn đã quên thêm thành phần "${action}" cho tài nguyên "${resource?.name}".`
        )
      );
    }
      ...
      <div className={cn("text-center", "space-y-8")}>
        ...
        <div className={cn("space-y-4")}>
          <h1 className={cn("text-2xl", "font-semibold", "text-foreground")}>
            {translate("pages.error.title", "Không tìm thấy trang.")}
          </h1>
        ...
        >
          ...
          {translate("pages.error.backHome", "Về trang chủ")}
        </Button>
      </div>
    </div>
  );
